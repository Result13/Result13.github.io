...
document.addEventListener('DOMContentLoaded', function () {
  var tabTypes = ['new', 'hit'];
  tabTypes.forEach(function (type) {
    var tabs = document.querySelectorAll("ul.".concat(type, "__tabs li"));
    tabs.forEach(function (tab, index) {
      tab.addEventListener('click', function () {
        // Toggle active class on tabs
        tabs.forEach(function (innerTab, innerIndex) {
          innerTab.classList.toggle("".concat(type, "__tab_active"), index === innerIndex);
        });

        // Toggle active class on contents and refresh sliders
        var contents = this.closest('div.container').querySelectorAll("div.".concat(type, "__content"));
        contents.forEach(function (content, contentIndex) {
          var isActive = index === contentIndex;
          content.classList.toggle("".concat(type, "__content_active"), isActive);
          if (isActive) {
            var sliders = content.querySelectorAll('.slider, .small__slider');
            sliders.forEach(function (slider) {
              $(slider).slick('refresh');
...
var rangeInput = document.querySelectorAll('.filter__range input'),
...
rangeInput.forEach(function (input) {
  input.addEventListener("input", function (e) {
    minVal = parseInt(rangeInput[0].value), maxVal = parseInt(rangeInput[1].value);
    if (maxVal - minVal < priceGap) {
      if (e.target.className === "filter__range__min") {
        rangeInput[0].value = maxVal - priceGap;
      } else {
...
filterWrapper.addEventListener('click', function (e) {
  ...
  var percentage = x / width; // percentage of the click position in the element
  var max = parseInt(rangeInput[1].max);
  var min = parseInt(rangeInput[0].min);
  var value = Math.round((max - min) * percentage + min); // value to be set for the slider

  // calculate which thumb is closer to the click position
  var middle = (parseInt(rangeInput[0].value) + parseInt(rangeInput[1].value)) / 2;
  if (value < middle) {
