...
document.querySelectorAll('.currency_button').forEach(function(button) {
  button.addEventListener('click', function() {
    var menu = this.nextElementSibling;
    var img = this.querySelector('.btn_img'); // Найти тег img с классом btn_img внутри кнопки
    
    if (menu.style.display === "none") {
      menu.style.display = "block"; // Показать список валют
      img.src = "/visi2/pictures/Vector.svg"; // Перевернуть картинку
    } else {
      menu.style.display = "none"; // Скрыть список валют
      img.src = "/visi2/pictures/Vectorrot.svg"; // Вернуть картинку в исходное положение
    }
  });
});
var prices = {
...
document.querySelectorAll('.currency_list li').forEach(function(li) {
  li.addEventListener('click', function() {
    ...
    currencyBlock.querySelector('.currency_flag').src = "/visi2/pictures/" + this.textContent+".svg";
    
    // Скрыть список валют
    this.parentElement.style.display = "none";
    
    // Вернуть картинку в исходное положение
    currencyBlock.querySelector('.currency_button .btn_img').src = "/visi2/pictures/Vectorrot.svg";
    ...
    if (card.classList.contains('active')) {
      ...
      if (cards[i + 1]) {
        ...
        if (i + 1 === cards.length - 1) {
          ...
          document.querySelector('.line_progress').style.display = 'none';
...
document.querySelector('.button_next').addEventListener('click', function() {
  ...
  var lineFill = document.querySelector('.line_fill');
  var currentWidth = parseInt(lineFill.style.width);
  if (isNaN(currentWidth)) {
    currentWidth = 122; // начальная ширина
  }
  
  var newWidth = currentWidth + 122;
  if (newWidth > 490) {
    newWidth = 490; // максимальная ширина
  }
  
  lineFill.style.width = newWidth + 'px';
  var lineFillMobile = document.querySelector('.line_fill_mobile');
  var currentWidthMobile = parseInt(getComputedStyle(lineFillMobile).width);
  if (isNaN(currentWidthMobile)) {
    currentWidthMobile = 79; // начальная ширина для .line_fill_mobile
  }

  var newWidthMobile = currentWidthMobile + 79;
  if (newWidthMobile >= 315) {
    newWidthMobile = 315; // максимальная ширина
  }

  lineFillMobile.style.width = newWidthMobile + 'px';
});
...
Papa.parse("../csv/base.csv", {
  ...
  complete: function(results) {
    ...
    if (titles.hasOwnProperty(campaign_id)) {
      ...
      if (title === "") {
        document.querySelector('.subtittle_text').style.display = 'none';
      }
    }
    // Если номер существует в словаре, изменить текст заголовка
